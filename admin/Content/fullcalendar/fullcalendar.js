...
$.extend(BasicView.prototype, {
	...
	numberCellHtml: function(row, col, date) {
	    ...
	    var htmlValue = "<span class='solarday'>" + cnDay + "</span><span class='holiday'>" + holiday + "</span>";
		//var htmlValue = '';
		return '' +
...
function RunGLNL() {
    var today = new Date();
    //var d = new Array("週一", "週二", "週三", "週四", "週五", "週六", "週日");
...
function DaysNumberofDate(DateGL) {
    var _value;
    try {
        _value = parseInt((Date.parse(DateGL) - Date.parse(DateGL.year() + "/1/1")) / 86400000) + 1;
    }
    catch (err) {
        //console.log(DateGL);
...
function CnDateofDateStr(DateGL) {
    if (CnMonthofDate(DateGL) == "零月") return "　請調整電腦日期!";
...
        "寒露", "霜降", "立冬", "小雪", "大雪", "冬至");
    ...
    if (DateGL._d.getDate() > BeginTime.getDate()) {
        BeginTime.setTime(BeginTime.getTime() + DifferenceInMonth[M] * 1000);
        M++;
    }
